...
weibo_content <- read.csv("result_new.csv", header = FALSE, sep = ",")
colnames(weibo_content)
dim(weibo_content)
head(weibo_content,5)

#some errors are spotted. There should be only 5 columns, but there are~1000 columns with non-empty values
...
all_weibo <- read.csv("all_weibo.csv", header = TRUE, sep = ",")
